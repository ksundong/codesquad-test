...
8. 클래스 및 객체에서 사용하기 위해 상수의 위치를 global로 이동하였습니다.

9. Player 클래스에 bat 메소드를 생성하였습니다. 정상동작을 확인하기 위한 코드를 포함하였습니다.

10. 초와 말을 표현하기 위한 상수를 global로 정의하였습니다.
...
13. Player의 정보를 보여줄 showInfo 메소드를 추가하였습니다.

14. 마지막 타석을 기억하여 다음 공격순서에 마지막 타자 이후의 타자가 타석에 올라갈 수 있도록 하였습니다. (lastbatter)

15. Inning의 정보를 보여줄 showInfo 메소드를 추가하였습니다.

16. Inning을 수행하는 기능을 playInning으로 추출하였고, 반복문으로 초와 말을 수행하도록 하였습니다.

17. out이 3회가 되기 전까지 타자가 계속 타석에 올라가도록 while문을 사용하였습니다.

18. log를 남기는 log메소드에 매개변수를 추가하여 변경된 코드에서 동작하도록 수정하였습니다.

19. Player가 타석을 수행하는 기능을 playBatting 메소드로 추출하였습니다.

20. update, handle 메소드에 매개변수를 추가하여 변경된 코드에서 동작하도록 수정하였습니다.

21. 경기 종료시 예시와 같이 점수정보를 보여주도록 코드를 추가하였습니다.

22. 득점시 득점을 했다고 알려주도록 코드를 추가하였습니다.

23. 승부 결과를 알려주는 메소드를 추가하였습니다.

## Step 3

1. 각 팀의 투구 수 삼진 수, 안타 수를 기록하기 위해 Team 클래스에 pitches와 strikeouts, hit를 생성하였습니다.

2. 팀의 이닝별 득점을 기록하기 위한 scoreHistory 배열을 선언하였고 생성시 0으로 초기화 하도록 하였습니다.

3. 공격팀인지 여부를 알 수 있도록 Team 클래스에 isAttackTeam이라는 불린형 변수를 선언하였습니다.
...
6. 전광판에 현재 이닝의 상태를 보여주는 부분을 추가하였습니다.

7. 6회초가 끝난 후 6회말 시작 시 팀2가 승리하고 있다면 바로 경기가 종료되도록 하였습니다.

8. 회차를 12회까지 저장하던 것을 6회까지만 저장하도록 수정하고 점수판의 모양을 다듬었습니다.

9. 이닝을 스킵하는 기능을 만들었습니다. 스킵할 이닝의 회를 입력받는 skipNum을 선언하였습니다.  
   skipInning 메소드를 만들었습니다. skipNum회 말까지는 점수표만 계속 출력되고 투구는 나오지 않습니다.  
   투구 보기의 입력값을 체크하는 inputChecking 메소드를 만들었습니다.

10. UI예시와 동일하게 player의 정보를 보여주는 항목을 보완하였습니다.

리팩토링.

1. var로 변수가 선언되어있던 것을 let으로 변경하였습니다.

2. 비교연산자를 ==에서 ===으로 변경하였습니다.

3. 파라미터 명을 innning에서 inning으로 변경하였습니다.

4. 전역변수를 constants.js 파일에서 선언하고 require하는 방식으로 수정하였습니다.
   가독성 및 클래스 분리시 재사용성을 높이기 위함입니다.
